...
def single_instance_eval(gen_first_sent, prompt_wiki_names, run_nli_metric, run_ner_metric, first_sentence_only=True, verbose=False):
...
def main(args):
    ...
    if args.gen_path != None:
        if len(args.eval_dir) > 0:
            gen_path = "{}/{}".format(args.eval_dir, args.gen_path)
        else:
    ...
    for _, (prompt_obj, gen_obj) in tqdm(enumerate(zip(prompts, gens)), total=len(prompts)):
        ...
        for i in range( len(sents[:args.num_eval_sent])):
            res_obj = single_instance_eval(sents[i], prompt_wiki_names, run_nli_metric, run_ner_metric)
    ...
    output_folder = os.path.dirname(gen_path) + '/' + score_folder_name
    if not os.path.exists(output_folder):
        os.makedirs(output_folder)
    res_path = os.path.dirname(gen_path) + '/'+score_folder_name+'/' + os.path.basename(gen_path).replace(".jsonl", "_results.jsonl")
